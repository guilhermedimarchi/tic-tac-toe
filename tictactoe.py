...
def max_value(board, cutoff):
    ...
    for action in actions(board):
        temp, act = min_value(result(board, action), v)
        if temp > v:
            v = temp
            move = action

        # Alpha-beta pruning. No need to check further nodes if next will be worse
        if v > cutoff:
            break
...
def min_value(board, cutoff):
    ...
    move = None
    for action in actions(board):
        temp, act = max_value(result(board, action), v)
        if temp < v:
            v = temp
            move = action
